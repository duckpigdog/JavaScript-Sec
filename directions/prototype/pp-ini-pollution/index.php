...
if (isset($_GET['ajax'])) {
    ...
    try {
        if (!isset($_FILES['iniFile']) || $_FILES['iniFile']['error'] !== UPLOAD_ERR_OK) {
            echo json_encode(['error' => '未上传文件或上传失败'], JSON_UNESCAPED_UNICODE);
            exit;
        }
        $tmpPath = $_FILES['iniFile']['tmp_name'];
        $ini = @parse_ini_file($tmpPath, true, INI_SCANNER_RAW);
        if ($ini === false) {
            echo json_encode(['error' => 'INI 解析失败'], JSON_UNESCAPED_UNICODE);
            exit;
        }
        $hasPwn = false;
        $scan = function($obj) use (&$scan, &$hasPwn) {
            ...
            foreach ($obj as $k => $v) {
                if ($k === 'iniPwn' && $v === 'yes') { $hasPwn = true; return; }
        ...
        if ($hasPwn) {
            echo json_encode([
                'result' => 'INI 解析触发，检测到 iniPwn === "yes"',
                'data' => $ini,
            ], JSON_UNESCAPED_UNICODE);
            exit;
        }
        echo json_encode(['result' => '未检测到 iniPwn === "yes"', 'data' => $ini], JSON_UNESCAPED_UNICODE);
    } catch (Throwable $e) {
  ...
  <body>
    <div id="app" class="app-shell">
      <main class="container">
        <section class="hero">
          <h1 class="glow">INI 解析原型污染</h1>
          <p class="tagline">解析 INI 文件时污染原型链</p>
        </section>
        ...
        <section class="panel" id="injectPanel">
          ...
          <input type="file" id="iniFile" name="iniFile" accept=".ini,.txt" class="input" />
          <div style="margin-top: 10px; display:flex; gap:10px; flex-wrap:wrap;">
            <button id="uploadBtn" class="btn">上传并解析</button>
        ...
        <section id="secretPanel" class="panel hidden" style="margin-top:16px;">
          <div>受保护的内容</div>
          <div style="margin-top:8px;">CTF<div>CTF Flag：<span style="color: var(--accent)">FLAG{PP_INI_POLLUTION_SUCCESS}</span></div>
        </section>
      </main>
    </div>
    ...
    <script>
          ...
          .then(async (res) => {
            try {
              ...
              if (data && data.data && typeof data.data === 'object') {
                ...
                const gate = {};
                const allow = gate.iniPwn === 'yes';
                document.getElementById('secretPanel').classList.toggle('hidden', !allow);
              } else {
      ...
      document.getElementById('resetBtn').addEventListener('click', () => {
        delete Object.prototype.iniPwn;
